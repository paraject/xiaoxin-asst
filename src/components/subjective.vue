...
export default {
  ...
  methods: {
    loadOther(provider, rank) {
      ...
      fetch('https://zuoyenew.xinkaoyun.com:30001/holidaywork/student/getMutualTaskInfo', {
        method: 'post',
        headers: {
  ...
  watch: {
    userInfo() {
      //年级排名
      fetch('https://zuoyenew.xinkaoyun.com:30001/holidaywork/student/getGradeRanks', {
        method: 'post',
      ...
      fetch('https://zuoyenew.xinkaoyun.com:30001/holidaywork/student/getClassRanks', {
        method: 'post',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: this.Qs.stringify({
          token: this.userInfo.token,
      ...
      fetch('https://zuoyenew.xinkaoyun.com:30001/holidaywork/getQiNiuToken')
        .then((res) => res.json())
        .then((res) => {
          if (res.state == 'ok') {
...
<template>
  <div v-if="subjectNum > 0">
    <div class="bg-green-200 dark:bg-green-600 shadow overflow-hidden rounded-lg mt-5 -mb-4">
      <div class="pt-4 pb-8 px-4 md:px-6 text-lg font-bold text-green-700 dark:text-white">
        主观题
        <div class="float-right">
          <label class="btn btn-primary btn-sm text-white border-none bg-green-700" for="manual" @click="cleanProvideSubject()">手动上传</label>
          <label class="btn btn-primary btn-sm ml-2 mr-0 text-white border-none bg-green-700" @click="this.randomOther()" for="openShow">随机二卷</label>
        </div>
      </div>
    </div>
    ...
    <table class="table table-compact w-full text-center shadow-md rounded-lg">
      ...
      <thead>
        <tr class="text-green-700">
          <th class="bg-green-50 dark:bg-green-700 dark:text-white rounded-tl-lg">排名</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white"><input type="text" placeholder="姓名" class="dark:bg-zinc-800 input input-bordered input-success input-sm w-20" v-model="this.searchName" @change="this.listUpdate()" /></th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white">班级</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white">分数</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white rounded-tr-lg">
            <label class="swap">
              <input
                type="checkbox"
                @click="
      ...
      <tbody v-if="this.searchName" class="rounded-b-lg">
        <tr v-for="(paper, index) in this.listNow" :key="index" class="rounded-b-lg">
          ...
          <td><label class="btn btn-outline btn-primary btn-sm" @click="this.loadOther(paper, index)" for="openShow">浏览二卷</label></td>
        </tr>
      </tbody>
      ...
      <tfoot>
        <tr class="text-green-700">
          <th class="bg-green-50 dark:bg-green-700 dark:text-white rounded-l-lg">排名</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white"><input type="text" placeholder="姓名" class="dark:bg-zinc-800 input input-bordered input-success input-sm w-20" v-model="this.searchName" @change="this.listUpdate()" /></th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white">班级</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white">分数</th>
          <th class="bg-green-50 dark:bg-green-700 dark:text-white rounded-r-lg">
            <label class="swap">
              <input
                type="checkbox"
                @click="
      ...
      <div class="modal">
        <div class="modal-box relative w-full max-w-full rounded-none max-h-full h-full bg-white dark:bg-zinc-800 overflow-x-hidden md:px-36 md:py-8 px-6 py-3">
          <label class="btn btn-ghost btn-circle text-gray-900 text-base mb-3 mr-1 p-2 float-left dark:text-white" for="openShow" @click="cleanProvideSubject()"
            ><svg t="1669730573158" class="icon w-5 h-5" viewBox="0 0 1307 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5572" width="48" height="48">
              <path d="M268.70922 566.468085h929.588652c36.312057 0 72.624113-29.049645 72.624114-72.624113 0-36.312057-29.049645-72.624113-72.624114-72.624114H305.021277l297.758865-297.758865c29.049645-29.049645 29.049645-72.624113 0-101.673759-29.049645-29.049645-72.624113-29.049645-101.673759 0L72.624113 450.269504c-14.524823 14.524823-21.787234 36.312057-21.787234 58.09929 0 21.787234 0 43.574468 21.787234 58.099291l428.48227 428.48227c29.049645 29.049645 72.624113 29.049645 101.673759 0 29.049645-29.049645 29.049645-72.624113 0-101.673759l-334.070922-326.808511z" fill="currentColor" p-id="5573"></path>
            </svg>
          </label>
          <label class="btn btn-ghost btn-circle text-[#595959] text-base mb-3 p-2 float-left dark:text-zinc-300" @click="this.randomOther()"
            ><svg t="1669733067993" class="icon w-5 h-5" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="7058" width="48" height="48"><path d="M187.456 425.024a336 336 0 0 0 368.384 420.224 48 48 0 0 1 12.544 95.168 432 432 0 0 1-473.664-540.16l-57.28-15.36a12.8 12.8 0 0 1-6.272-20.928l159.168-179.456a12.8 12.8 0 0 1 22.144 5.888l48.064 235.072a12.8 12.8 0 0 1-15.808 14.912l-57.28-15.36zM836.48 599.04a336 336 0 0 0-368.384-420.224 48 48 0 1 1-12.544-95.168 432 432 0 0 1 473.664 540.16l57.28 15.36a12.8 12.8 0 0 1 6.272 20.928l-159.168 179.456a12.8 12.8 0 0 1-22.144-5.888l-48.064-235.072a12.8 12.8 0 0 1 15.808-14.912l57.28 15.36z" fill="currentColor" p-id="7059"></path></svg>
          </label>
          <div class="font-bold md:mt-5 mt-3 dark:text-white">
            {{ this.nowSubjectProvider.className }}-{{ this.nowSubjectProvider.realName }}
          ...
          <div class="mt-5" v-for="(one, num) in this.choosenSubjectData" :key="one.teaCode">
              ...
              <div class="shadow-md grid relative z-10 my-4 -mx-4 sm:-mx-0 rounded-lg">
                ...
                <div class="card-body p-0">
                  <div class="card-actions justify-end">
                    <button @click="this.deleteChoosenImage(num, index)" class="-mt-16 mr-4 z-30 block p-3 text-green-700 transition-all bg-green-100 border-2 border-white rounded-full hover:scale-110 focus:outline-none focus:ring active:bg-green-50" type="button">
            ...
            <div v-for="(b, index) in this.fileList" :key="index">
              <div class="shadow-md grid relative z-10 my-4 -mx-4 sm:-mx-0 rounded-lg" v-if="b.teaCode == one.teaCode">
                ...
                <div class="card-body p-0">
                  <div class="card-actions justify-end">
                    <button @click="this.deleteSelfImage(index)" class="-mt-16 mr-4 z-30 block p-3 text-green-700 transition-all bg-green-100 border-2 border-white rounded-full hover:scale-110 focus:outline-none focus:ring active:bg-green-50" type="button">
                      <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                      </svg>
                    </button>
      ...
      <div class="modal justify-start">
        <div class="modal-box overflow-x-hidden w-full max-w-full rounded-none max-h-full h-full bg-white dark:bg-zinc-800 md:px-36 md:py-8 px-6 py-3">
          <label class="btn btn-ghost btn-circle text-gray-900 text-base mb-3 float-left dark:text-white" for="manual" @click="cleanProvideSubject()"
            ><svg t="1669730573158" class="icon w-5 h-5" viewBox="0 0 1307 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5572" width="48" height="48">
              <path d="M268.70922 566.468085h929.588652c36.312057 0 72.624113-29.049645 72.624114-72.624113 0-36.312057-29.049645-72.624113-72.624114-72.624114H305.021277l297.758865-297.758865c29.049645-29.049645 29.049645-72.624113 0-101.673759-29.049645-29.049645-72.624113-29.049645-101.673759 0L72.624113 450.269504c-14.524823 14.524823-21.787234 36.312057-21.787234 58.09929 0 21.787234 0 43.574468 21.787234 58.099291l428.48227 428.48227c29.049645 29.049645 72.624113 29.049645 101.673759 0 29.049645-29.049645 29.049645-72.624113 0-101.673759l-334.070922-326.808511z" fill="currentColor" p-id="5573"></path>
            </svg>
          </label>
          <div class="font-bold md:mt-5 mt-3 dark:text-white">手动上传</div>
          ...
          <div class="mt-10" v-for="(one, index) in this.taskData" :key="index">
            <div v-if="one.hasSubjectiveItem == 1" class="flex-col flex">
              ...
              <div v-if="this.fileList.length == 0" class="mt-4 my-32 dark:text-white">待上传...</div>
            </div>
          </div>
          <div class="mt-16">
    ...
    <div class="text-center mt-5">
      <button class="btn btn-primary mr-3 text-white" v-if="this.listGrade && !this.searchName" @click="this.subjectMaxLoad = this.subjectMaxLoad + 100 > this.graderank.length ? this.graderank.length : this.subjectMaxLoad + 300">加载更多 [{{ this.subjectMaxLoad }}/{{ this.graderank.length }}]</button>
      <button class="btn btn-secondary text-white" v-if="this.listGrade && !this.searchName" @click="this.subjectMaxLoad = this.graderank.length">加载全部</button>
    </div>
  </div>
</template>
