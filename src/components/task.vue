...
export default {
  ...
  methods: {
    ...
    submitTask() {
      fetch('https://zuoyenew.xinkaoyun.com:30001/holidaywork/student/submitTask', {
        method: 'post',
        headers: {
...
<template>
  <div id="box">
    ...
    <div class="px-2 md:px-16 mb-4 pt-4">
      <button class="mr-2 bg-white rounded-full p-2 hover:bg-gray-200 dark:bg-zinc-700" @click="backhome()">
        <svg t="1669622375669" class="icon w-4 h-4 text-secondary" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5305" width="32" height="32">
          <path d="M932.7 417.5c-17-38-41.1-72-71.9-101.3-30.5-29-65.9-51.7-105.2-67.6C715 232.3 672.1 224 627.9 224h-139V101.2c0-27.9-30.4-45.2-54.4-31L116.8 257.9c-23.6 14-23.6 48.1 0 62.1l317.7 187.7c24 14.2 54.4-3.1 54.4-31V364h139c51.5 0 100 19.1 136.5 53.8 35.5 33.8 55.1 78.1 55.1 124.9v98.1c0 46.8-19.6 91.2-55.1 124.9-36.5 34.7-85 53.8-136.5 53.8H397.1c-51.5 0-100-19.1-136.5-53.8-35.5-33.8-55.1-78.1-55.1-124.9 0-38.7-31.3-70-70-70s-70 31.3-70 70c0 43.3 9 85.4 26.7 125.1 17 38 41.1 72 71.9 101.3 30.5 29 65.9 51.7 105.2 67.6 40.6 16.4 83.6 24.7 127.8 24.7h230.7c44.2 0 87.2-8.3 127.8-24.7 39.3-15.9 74.7-38.6 105.2-67.6 30.7-29.2 54.9-63.3 71.9-101.3 17.7-39.7 26.7-81.8 26.7-125.1v-98.1c0-43.4-9-85.5-26.7-125.2z" p-id="5306" fill="currentColor"></path>
        </svg>
      </button>
      <span class="text-gray-700 font-bold mr-2 text-lg dark:text-white"> {{ taskInfo.taskName }}</span>

      <div class="badge badge-secondary px-2.5 py-0.5 font-bold text-white" v-if="taskInfo.submitCode == 0">{{ submitStatus }}</div>
      <div class="badge badge-primary text-white px-2.5 py-0.5 font-bold" v-if="taskInfo.submitCode == 1">{{ submitStatus }}</div>
      <div class="badge badge-secondary badge-outline px-2.5 py-0.5 font-bold" v-if="taskInfo.submitCode == 2">{{ submitStatus }}</div>

      <objective :taskInfo="taskInfo" :userInfo="userInfo" :taskData="taskData" :objectNum="objectNum" :subjectNum="subjectNum" ref="objectiveRef"></objective>
      <subjective :taskInfo="taskInfo" :userInfo="userInfo" :taskData="taskData" :objectNum="objectNum" :subjectNum="subjectNum" @submit-task="preSubmitTask"></subjective>
    ...
    <div class="modal">
      <div class="modal-box relative w-full max-w-full rounded-none max-h-full h-full bg-white dark:text-white overflow-x-hidden dark:bg-zinc-800">
        <h3 class="text-xl font-bold mt-4">仅差一步，请核对您的作业提交数据</h3>

        <div class="flex bg-red-100 dark:bg-red-600 rounded-lg p-4 my-4 text-sm text-red-700 dark:text-white md:ml-5" role="alert">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="-ml-1 mr-1.5 w-5 h-5 inline">
        ...
        <div>
          <span class="badge badge-secondary mr-1 p-4 text-lg font-bold text-white my-1 uppercase md:ml-5" v-for="(obj, index) in objectData" :key="index">{{ obj }}</span>
        </div>
        ...
        <div class="flex flex-col justify-center items-center">
          <div class="md:w-2/3 sm:w-full rounded-lg shadow-lg bg-white my-3">
            ...
            <div class="px-10 py-5 text-gray-600 font-bold leading-6">
              ...
              <div class="sm:flex">
                ...
                <div class="w-48 sm:ml-4 sm:mt-1 mt-2">
                  ...
                  <a class="text-lg mt-3" href="https://qr.dingtalk.com/action/joingroup?code=v1,k1,mQvtrGj2v7QyElRcjOGFciRHyuduanL5u+OlefppD64=&_dt_no_comment=1&origin=11">
            ...
            <div class="px-5 py-4 flex justify-end">
              <button @click="backhome()" class="bg-green-500 mr-1 rounded text-sm py-2 px-3 text-white hover:bg-green-600 transition duration-150">回到主页</button>
            </div>
          </div>
        </div>
      </div>
